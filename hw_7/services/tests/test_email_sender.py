...
from services.email_templates.question_alert import email_template, html_email_temlate
...
class TestSender(TestCase):
    ...
    def test_sender_validation(self):
        sender = Sender(
            self.recipients, self.template, self.vars, self.subject, validate=False
        )
        self.assertTrue(sender.validate())

    def test_sender_creation(self):
        sender = Sender(self.recipients, self.template, self.vars, self.subject)
    ...
    def test_bad_sender_creation(self):
        sender1 = Sender(None, self.template, self.vars, self.subject, validate=False)
        sender2 = Sender(
            ["not an email"], self.template, self.vars, self.subject, validate=False
        )
        sender3 = Sender(self.recipients, {}, self.vars, self.subject, validate=False)
