...
class TestAnswerForm(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.question = Question(
            title="How to Django?", author=cls.user, content="Lorem ipsum dolor est"
        )
        cls.question.save()

    def test_answer_form_basics(self):
        form = AnswerForm(
            {"content": "This is some answer"}, question_id=self.question.id
        )
    ...
    def test_answer_content_clearing(self):
        ...
        """
        old_answer = Answer(
            author=self.user, question=self.question, content="This is the same answer"
        )
        old_answer.save()
        form = AnswerForm(
            {"content": "This is the same answer"}, question_id=self.question.id
        )
...
class TestQuestionForm(TestCase):
    ...
    def test_question_form_clean_tags_good(self):
        for choice in ("awfwa", "Python OTUS Django", "one two", ""):
            with self.subTest(choice=choice):
                ...
                self.assertTrue(form.is_valid())
                self.assertEqual(form.cleaned_data.get("tags"), choice.split())
