...
class TestQuestion(TestCase):
    ...
    def setUpTestData(cls) -> None:
        cls.sam = User.objects.create_user(
            username="Sam", email="sam@pisem.net", password="sampassword"
        )
        cls.sam.save()
        cls.alice = User.objects.create_user(
            username="Alice", email="alice@wonderland.com", password="alicepass"
        )
        cls.alice.save()
        cls.bob = User.objects.create_user(
            username="Bob", email="bob@bobpost.org", password="bobpass"
        )
        cls.bob.save()
    ...
    def setUp(self):
        self.q = Question(
            title="How to Django?", author=self.sam, content="Lorem ipsum dolor est"
        )
        self.q.save()
    ...
    def test_trending_logic(self):
        ...
        def make_questions(range):
            ...
            for _ in range:
                questions.append(
                    Question(
                        title=words(random.randint(3, 6), common=False)
        ...
        self.assertGreaterEqual(len(qws), settings.TRENDING_QUESTIONS_NUMBER + 15)
        self.assertGreaterEqual(
            Question.objects.all().count(), settings.TRENDING_QUESTIONS_NUMBER + 15
        )
...
class TestAnswers(TestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        # questions
        cls.qw1 = Question(
            title="How to Django?", author=cls.sam, content="Lorem ipsum dolor est"
        )
        cls.qw1.save()
        cls.qw2 = Question(
        ...
        cls.qw2.save()
        cls.qw3 = Question(
            title="How to OTUS?", author=cls.bob, content="Ipsum lorem dolor ne est"
        )
        cls.qw3.save()
    ...
    def test_answer_basic_creation(self):
        ...
        self.assertEqual(self.ans.content, "If lorem ipsum or dolor est, do django")
    ...
    def test_change_flag(self):
        ...
        self.assertEqual(self.qw1.answer_set.get(answer_flag=1), another_good_answer)
    ...
    def test_question_answers_number(self):
        ...
        answer2 = Answer(
            author=self.alice, question=self.qw2, content="Do that and do not do this"
        )
        answer2.save()
...
class TestVoters(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.sam.save()
        cls.alice = User.objects.create_user(
            username="Alice", email="alice@wonderland.net", password="alicepassword"
        )
        cls.alice.save()
        cls.q = Question(
            title="How to Django?", author=cls.sam, content="Lorem ipsum dolor est"
        ...
        cls.q.save()
        cls.a = Answer(author=cls.sam, question=cls.q, content=words(10, common=False))
        cls.a.save()
    ...
    def test_basic_votes_creation_question(self):
        ...
        self.assertEqual(
            Voters.objects.filter(
                content_type=object_ct, object_id=self.q.id, user_id=self.sam.id
            ).count(),
            1,
        )
        ...
        self.assertEqual(retrieved_vote.user_id, self.sam.id)
        self.assertEqual(retrieved_vote.object_id, self.q.id)
    ...
    def test_register_ans_upvote(self):
        ...
        retrieved_vote = Voters.objects.get(
            content_type=object_ct, object_id=self.a.id, user_id=self.sam.id
        )
        self.assertEqual(retrieved_vote.vote, 1)
    ...
    def test_register_assert_integrity_answers(self):
        ...
        def check(object: Answer or Question):
            ...
            retrieved_vote = Voters.objects.get(
                content_type=object_ct, object_id=object.id, user_id=self.sam.id
            )
